    ...
    public class RestrictionOperators
    {
        ...
        private class LinqSamples
        {
            ...
            public void Linq2()
            {
                ...
                var soldTest = products.Where(prod => prod.UnitPrice > 70).Select(x => new { x.ProductName }).ToList();

                var namesAndPrices = products.Where(p => p.UnitPrice >= 10).Select(p => new { p.ProductName, p.UnitPrice }).ToList();
                IEnumerable<int> indices =products.Select((product, index) => new { product, index }).Where(x => x.product.UnitPrice >= 10)
					.Select(x => x.index);
                ...
                var namesAndOrderIDs1 = customers.
                Where(c => c.Country == "Denmark").
                SelectMany(c => c.Orders, (c, o) => new { c, o }).
                Where(co => co.o.OrderDate.Year == 1997).Select(co => new { co.c.CompanyName, co.o.OrderID });
                ...
                var simpleQuery = products.Select(num => num.UnitPrice == 97.0000M);
